...
public class show_student_request extends AppCompatActivity {
    private MainOpenHelper mDB;
    private RecyclerView mRecyclerView;
    private RequestStudentPassListAdapter mAdapter;
    //List<PassStudent> studentList;
    List<Collage_Select_Data> studentList;
    private String astatus="INVALID";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //studentList =  mDB.makeStudentRequest();

        // mDB.onCreate();
        mRecyclerView = (RecyclerView) findViewById(R.id.recyclerviewRequestStudent);

        Call<Collage_Select> responseCall1 = RetrofitClient.getInstance()
                .getInterPreter().FindValidStudentList(astatus);
        responseCall1.enqueue(new Callback<Collage_Select>() {
            ...
            public void onResponse(Call<Collage_Select> call, Response<Collage_Select> response) {
                ...
                if(response1.error)
                {
                    Toast.makeText(show_student_request.this, "Error Occurred", Toast.LENGTH_LONG).show();
                }
                else{
                    studentList = response1.data;
                    mAdapter = new RequestStudentPassListAdapter(show_student_request.this,studentList);
                    mRecyclerView.setLayoutManager(new LinearLayoutManager(show_student_request.this));
                    mRecyclerView.setAdapter(mAdapter);

                    Toast.makeText(show_student_request.this, "successful",Toast.LENGTH_LONG).show();
        ...
        /*mAdapter = new RequestStudentPassListAdapter(this,studentList);
