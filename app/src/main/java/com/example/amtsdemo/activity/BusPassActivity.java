...
public class BusPassActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Call<Student_Status> responseCall1 = RetrofitClient.getInstance()
                .getInterPreter().FindStudentPassStatus(uid);
        responseCall1.enqueue(new Callback<Student_Status>() {
            @Override
            public void onResponse(Call<Student_Status> call, Response<Student_Status> response) {
                Student_Status response1 = response.body();
                if (response1.error) {
                    Toast.makeText(BusPassActivity.this, "Error Occurred....", Toast.LENGTH_LONG).show();
                } else {
                    strStatus = response1.data.get(0).getAstatus();
                    if(strStatus.matches("INVALID")){
                        textViewName.setText("Student Bus Pass Request is Pending");
                    ...
                    Toast.makeText(BusPassActivity.this, "successful", Toast.LENGTH_LONG).show();
                }

            }

            @Override
            public void onFailure(Call<Student_Status> call, Throwable t) {
                Toast.makeText(BusPassActivity.this, "Error Occurred", Toast.LENGTH_LONG).show();
            }
        });
